...
* Python
* Docker containers
**{url-influx}[InfluxDB (Time series database)]
...
image::https://user-images.githubusercontent.com/63274055/155844751-1c5013fc-8563-4f43-8605-a1ccb916cb6d.png[width=800]
====
''''
====
...
1.- CSV files to be worked on which are the time series dataset

2.- Then I am going to create a Python ingestion program. This program is going to take the csv file,

3.- and is going to write it into the InfluxDB database, hosted as a Docker container. Once the data is in InfluxDB, (where the data and aggregations are stored) 

4.- then I can set up a dashboard in Grafana, also hosted in a Docker container and query the data from InfluxDB

5.- The "Client" is the pc of the stakeholders from which the Grafana UI will be accessed to the AWS EC2 instance with an URL
