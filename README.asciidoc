...
We can set a `restart policy` to control whether the containers start automatically when they stop, or when we restart Docker. Also restart policies help that containers that are linked start in the correct order.

To configure the restart policies of my two containers I used the `--restart` flag when using the `docker update` command (`docker update` is used for containers that are already running). I used the value `unless-stopped` which configures the container to always restart unless it is explicitly stopped or Docker is restarted.

```bash
docker update --restart unless-stopped 7257a389cc72

# 7257a389cc72 its the container ID
```


== Public sharing of the Grafana dashboards for this project
...
services:
  ...
  grafana:
    ...
    volumes:
      - ./gfdata:/var/lib/grafana
```
This is the difference from the previous docker compose file I had out together:

image::https://user-images.githubusercontent.com/63274055/158197834-2005997c-2044-4e11-8787-1ac1ceaef2c6.png[width=800]
...
{url-dash-3}[Air Quality Pollutants Concentration - Berlin]
